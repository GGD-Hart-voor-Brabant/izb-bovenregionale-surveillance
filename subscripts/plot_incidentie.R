...
plot_incidentie <- function(data, kolom, ziekte = "totaal", exclusie = NULL, kolom_uitsplitsing = NULL, uitsplitsing = NULL){
  ...
  data <- mutate(data, jaarMaand = as.Date(paste0(jaarMaand, "-01"), format = "%Y-%m-%d")) %>% 
    ...
    # Maak een factor van de GGD kolom
    mutate(GGD = as.factor(GGD))
             
  ### Splits de data van de overkoepelende regio af
  Regio <- data[data$GGD %in% Regionaam,] %>%
    #specifiek label voor incidentie regio
    mutate(Incidentie_regio = Incidentie) %>% 
    ungroup() %>% 
    select(jaarMaand,Incidentie_regio)

  #Regiodata toevoegen aan subregio's dmv left_join
  data <- data %>%
    dplyr::filter(!GGD %in% Regionaam)%>%
    left_join(Regio, by = "jaarMaand")
  
  grafiek <- ggplot() +
    #Plot alle GGDen behalve Regiocijfers
    geom_line(data = data[!data$GGD %in% Regionaam,], 
              aes(x = jaarMaand, y = Incidentie, group = GGD, color = GGD),
              size = 1.5) + 
    # Plot de Regiocijfers
    geom_line(data = data[!data$GGD %in% Regionaam,], 
              aes(x = jaarMaand, y = Incidentie_regio, group = GGD, color = Regionaam),
              size = 1) + 
    ...
    scale_color_manual("GGD", breaks = levels(data$GGD),
                       values=c("BZO" = "#e41a1c", 
                                "HvB" = "#377eb8", 
                                "LN" = "#4daf4a", 
                                "WB" = "#984ea3",
